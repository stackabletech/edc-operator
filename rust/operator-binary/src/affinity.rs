          ...
          metastore:
            roleGroups:
              default:
                replicas: 1
        "#;
        let hive: EDCCluster = serde_yaml::from_str(input).expect("illegal test input");
        let merged_config = hive.merged_config(&EDCRole::Connector, "default").unwrap();
